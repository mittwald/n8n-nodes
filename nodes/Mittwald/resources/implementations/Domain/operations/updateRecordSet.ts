...
// The PUT body is a discriminated union over the record-set type (a/aaaa/cname/mx/srv/txt/caa/ns).
...
export default domainResource
	.addOperation({
		...
		description:
			'Update a record set on a DNS zone. The body is a discriminated union over the record-set type (a, aaaa, cname, mx, srv, txt, caa, ns) — provide the JSON-encoded body matching the chosen type. Refer to the mittwald API documentation for the exact shape.',
	})
	.withProperties({
		dnsZoneId: {
		...
		recordSetBody: {
			...
			required: true,
			description:
				'JSON-encoded request body matching the chosen record-set type. Refer to the mittwald API documentation for the exact shape.',
		},
	})
